...
begin
  ...
  InitWindow(screenWidth, screenHeight,
    StrToPChar('raylib [shaders] example - custom uniform variable'));
  ...
  dwarf := LoadModel(StrToPChar('res/models/dwarf.obj')); // Load OBJ model
  texture := LoadTexture(StrToPChar('res/models/dwarf_diffuse.png'));
  // Load model texture (diffuse map)
  //dwarf.material.maps[MAP_DIFFUSE].texture := texture;
  SetMaterialTexture(dwarf.materials, MAP_DIFFUSE, texture);
  // Set dwarf model diffuse texture

  position := Vector3Create(0.0, 0.0, 0.0); // Set model position

  shdr := LoadShader(StrToPChar('res/shaders/glsl330/base.vs'),
    StrToPChar('res/shaders/glsl330/swirl.fs')); // Load postpro shader

  // Get variable (uniform) location on the shader to connect with the program
  // NOTE: If uniform variable could not be found in the shader, function
  // returns -1
  swirlCenterLoc := GetShaderLocation(shdr, StrToPChar('center'));
  ...
  begin
    ...
    DrawText(StrToPChar('TEXT DRAWN IN RENDER TEXTURE'), 200, 10, 30, RED);
    ...
    DrawText(StrToPChar('(c) Dwarf 3D model by David Moreno'), screenWidth - 200,
